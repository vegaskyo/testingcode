...
public class ConnectionChecker2 {
    public static void main(String[] args) {
        ...
        try {
            ...
            for (String block : blocks) {
                ...
                JsonArray msgArray = rootObject.getAsJsonArray("msg");

                if (msgArray != null) {
                    for (JsonElement msgElement : msgArray) {
                        JsonObject msgObject = msgElement.getAsJsonObject();

                        String itemValue = msgObject.get("item").getAsString();

                        JsonArray stdoutLinesArray = msgObject.getAsJsonArray("stdout_lines");
                        if (stdoutLinesArray != null && stdoutLinesArray.size() > 0) {
                            String lastLine = stdoutLinesArray.get(stdoutLinesArray.size() - 1).getAsString();
